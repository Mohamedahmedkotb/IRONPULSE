...
function wireShellInteractions() {
  ...
  document.getElementById("sidebar-logout")?.addEventListener("click", async () => {
    ...
    window.location.href =
      typeof window.__IP_URLS__?.login === "string"
        ? window.__IP_URLS__.login
        : "login.html";
  });
...
export async function initLayout() {
  ...
  const serverAuth = document.body.dataset.serverAuth === "1";

  if (authRequired && shell === "dashboard" && !serverAuth) {
    try {
  ...
  if (shell === "dashboard") {
    ...
    const phpLayout = document.body.dataset.phpLayout === "1";
    if (!phpLayout) {
