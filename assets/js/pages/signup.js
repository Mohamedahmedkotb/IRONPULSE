...
function init() {
  ...
  signupForm?.addEventListener("submit", async (e) => {
    if (signupForm.dataset.php === "1") {
      ...
      const pw = pass instanceof HTMLInputElement ? pass.value : "";
      const hidGoal = document.getElementById("hid-fitness_goal");
      if (hidGoal instanceof HTMLInputElement) {
        hidGoal.value = selectedGoal;
      }
      const pe = document.getElementById("su-pass-err");
      if (pe) pe.hidden = true;
      if (pw.length < 8) {
      ...
      if (st.score < 2) {
        if (pe) {
          pe.textContent =
            "Choose a stronger password (mix letters, numbers, symbols).";
          pe.hidden = false;
        }
        return;
      }
